...
def fetch_google_events(service, start_date, end_date):
    ...
    # Fetch events from each calendar within the specified date range
    all_events = []
...
def insert_into_notion(client, events, calendar_mapping):
    ...
    temp_db = client.databases.query(
        **{
            ...
            "sorts": [{"property": "Entry Number", "direction": "descending"}],
        }
    )
    last_entry_no = temp_db["results"][0]["properties"]["Entry Number"]["title"][0][
        "plain_text"
    ]
    ...
    date_dict = generate_date_dict(end_date)
    ...
    client.pages.create(
        **{"parent": {"database_id": DATABASE_ID}, "properties": new_entry_dict}
    )
    return
...
def init_time_values(calendar_mapping):
    ...
    for item in calendar_mapping.values():
        new_entry = {"type": "number", "number": 0}
        output_dict.update({item["type"]: new_entry})

    return output_dict


def generate_date_dict(end_date):
    output_dict = {
        "type": "date",
        "date": {
...
def generate_entry_dict(last_entry_no):
    output_dict = {
        ...
        "title": [
            {
                "type": "text",
                "text": {"content": str(last_entry_no + 1), "link": None},
                "annotations": {
                    "bold": False,
                    "italic": False,
                    "strikethrough": False,
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Fetch and insert Google Calendar events into Notion."
    )
    parser.add_argument(
